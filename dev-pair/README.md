...
python3 <hermes-home>/devpair/test_devpair.py    # 360 checks, no network
```
...
```bash
python3.11 test_devpair.py     # 70 regression tests (360 checks), no network required
```
...
Current: **1.1.19**. See [CHANGELOG.md](CHANGELOG.md) — semver, patch (+0.0.1) per published change.
