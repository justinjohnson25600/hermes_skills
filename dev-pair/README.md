...
python3 <hermes-home>/devpair/test_devpair.py    # 351 checks, no network
```
...
- **Secrets are redacted before they leave your machine — from the whole prompt, not just the evidence.** The gathered files and diffs are scrubbed, and so is the assembled prompt around them: your `--ask`, your `--focus`, and the prior turns replayed into a `followup`. That last path mattered most — a reviewer that quoted a credential back at you in turn 1 used to re-send it verbatim on every later turn of the session. Everything passes through `redact_secrets()`: vendor token shapes (`sk-`, `ghp_`, `AKIA`, `xox`, `AIza`, `ya29`, JWTs), private-key blocks, passwords embedded in URLs, secret-looking `KEY=value` assignments, and bearer tokens in `Authorization` headers are replaced with `[REDACTED:kind]`, and you're told how many were caught, and whether they came from the evidence or from your own question. Key *names* and URL hosts survive so the reviewer can still reason about the code. Obvious placeholders (`<your-key-here>`, `changeme`) are left alone.
...
```bash
python3.11 test_devpair.py     # 68 regression tests (351 checks), no network required
```
...
Current: **1.1.18**. See [CHANGELOG.md](CHANGELOG.md) — semver, patch (+0.0.1) per published change.
