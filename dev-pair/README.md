...
python3 <hermes-home>/devpair/test_devpair.py    # 305 checks, no network
```
...
```bash
python3.11 test_devpair.py     # 64 regression tests (305 checks), no network required
```
...
Current: **1.1.14**. See [CHANGELOG.md](CHANGELOG.md) — semver, patch (+0.0.1) per published change.
